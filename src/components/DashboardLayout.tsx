...
export default function DashboardLayout() {
  ...
  return (
    <div className="min-h-screen flex bg-[#fafafa]" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      
      {/* Mobile Header (Glassmorphism) */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-[60px] bg-white/80 backdrop-blur-md border-b border-neutral-200/60 z-30 flex items-center justify-between px-4">
        <button 
          onClick={() => setSidebarOpen(true)}
          className="w-10 h-10 flex items-center justify-center text-neutral-600 rounded-full hover:bg-neutral-100/80 active:bg-neutral-200 transition-colors"
      ...
      <aside 
        className={`
          fixed inset-y-0 left-0 z-50 w-[260px] bg-white border-r border-neutral-200/60 shadow-[4px_0_24px_rgba(0,0,0,0.02)]
          flex flex-col transform transition-transform duration-300 ease-out
          ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}
          md:relative md:translate-x-0
        `}
      >
        <div className="h-[60px] md:h-[72px] flex items-center justify-between px-6 border-b border-neutral-100 shrink-0">
          ...
          <button 
            className="md:hidden w-8 h-8 flex items-center justify-center text-neutral-400 hover:text-neutral-900 rounded-full hover:bg-neutral-100 transition-colors"
            onClick={() => setSidebarOpen(false)}
          >
            <X size={20} />
          </button>
          ...
          {navItems.map(item => {
            ...
            return (
              <Link
                key={item.to}
                to={item.to}
                className="group relative flex items-center gap-3 px-3 h-[44px] rounded-lg text-[14px] font-medium transition-all duration-200"
                style={{
              ...
              >
                {isActive && (
                  <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 bg-[#171717] rounded-r-full shadow-[0_0_8px_rgba(23,23,23,0.3)]" />
                )}
                
                <div className={`
                  flex items-center justify-center w-8 h-8 rounded-md transition-all duration-200
                  ${isActive ? 'bg-white shadow-sm text-black' : 'text-neutral-400 group-hover:text-neutral-700 group-hover:bg-neutral-100 group-active:scale-95'}
                `}>
                  <item.icon size={18} strokeWidth={isActive ? 2.5 : 2} />
                </div>
                
                <span className={`transition-transform duration-200 ${isActive ? 'translate-x-0.5' : 'group-hover:translate-x-1'}`}>
                  {item.label}
                </span>
              </Link>
            );
        ...
        <div className="p-4 border-t border-neutral-100 shrink-0">
          <button
            ...
            className="group flex items-center gap-3 px-3 h-[44px] w-full rounded-lg text-[14px] font-medium transition-all duration-200 text-neutral-500 hover:text-red-600 hover:bg-red-50 active:scale-[0.98]"
          >
            <div className="flex items-center justify-center w-8 h-8 rounded-md transition-colors group-hover:bg-red-100">
              <LogOut size={18} />
            </div>
      ...
      <main className="flex-1 overflow-x-hidden overflow-y-auto pt-[60px] md:pt-0 scroll-smooth">
        <div className="p-4 sm:p-6 md:p-8 max-w-5xl mx-auto w-full min-h-full">
          <Outlet />
        </div>
      </main>
    </div>
